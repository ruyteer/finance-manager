...
export default function CreditCardManager({
  ...
  const cardMetrics = useMemo(() => {
    ...
    const currentStatementTransactions = cardTransactions.filter((t) => {
      ...
      return (
        transactionDate > previousClosingDate && transactionDate <= closingDate
      );
    });

    // Calculate current statement amount
    const currentStatementAmount = currentStatementTransactions.reduce(
      (sum, t) => sum + t.amount,
      0
    );
    ...
    const usagePercentage =
      card.limit > 0 ? (currentStatementAmount / card.limit) * 100 : 0;
  ...
  const confirmDelete = async () => {
    if (cardToDelete) {
      ...
      if (cardToDelete === selectedCard) {
        ...
        setSelectedCard(
          remainingCards.length > 0 ? remainingCards[0].id : null
        );
      }
    }
  };
      ...
      <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-slate-400 mb-6">
            Adicione seu primeiro cartão de crédito para começar a gerenciar
            suas faturas.
          </p>
        </CardContent>
      </Card>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="text-sm text-purple-400">
                              {cardMetrics.usagePercentage.toFixed(0)}%
                            </div>
                          </div>
                          <Progress
                            value={cardMetrics.usagePercentage}
                          ...
                          >
                            <div
                              ...
                              style={{
                                width: `${cardMetrics.usagePercentage}%`,
                              }}
                            />
                          </Progress>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-2">
                          ...
                          <div>
                            ...
                            <p className="text-lg font-medium text-green-400">
                              {formatCurrency(
                                card.limit - cardMetrics.currentStatementAmount
                              )}
                            </p>
                          </div>
                        </div>
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <p className="text-lg font-medium text-slate-200">
                            Dia {card.closingDay} de cada mês
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className="text-lg font-medium text-amber-400">
                            {formatDate(cardMetrics.dueDate.toString())}
                          </p>
                        </div>
                      </div>
                    </CardContent>
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-medium text-slate-200">
                            {cardMetrics.transactions.length} itens
                          </p>
                        </div>
                      </div>
                    </CardContent>
        ...
        <AlertDialogContent className="bg-slate-800 border-slate-700">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              associadas a ele permanecerão, mas não estarão mais vinculadas a
              um cartão.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
